...
const Header = () => {
    ...
    const handleAuthModal = () => setShowAuthModal(!showAuthModal);
                ...
                <Section>
                    <nav className="flex gap-[5rem] py-[1rem] max-md:gap-[1.5rem] max-sm:grid max-sm:grid-cols-2">
                        ...
                        <div className="flex max-lg:gap-[1.5rem] max-sm:justify-end">
                            <Button type="dark" onClick={handleAuthModal}>Войти</Button>
                            <MenuOpenButton onClick={handleMenu}/>
                        </div>
                    </nav>
                </Section>
            </header>
            {/* REPLACE THE BASE MODAL WITH LOGIN FORM MODAL */}
            {
                showAuthModal && (
                    <BaseModal title='Рады вас снова видеть' onClose={handleAuthModal}>
                        <LoginForm />
                    </BaseModal>
                )
            }
